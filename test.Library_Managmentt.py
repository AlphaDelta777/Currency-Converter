import os
import pytest
from Library_Managment import Library, MediaFactory, Member, Book, EBook, AudioBook, Observer
...
def sample_book():
    return MediaFactory.create(
        "book", id=10, title="Test Book", author="Author A", year=2020,
        isbn="111-222", pages=200, genre="Fiction"
    )

@pytest.fixture
def sample_ebook():
    return MediaFactory.create(
        "ebook", id=11, title="Test EBook", author="Author B", year=2021,
        file_size=3.5, format="PDF"
    )
...
def test_media_factory_creates_correct_types():
    book = MediaFactory.create("book", id=1, title="B", author="A", year=2000, isbn="1", pages=10, genre="G")
    ebook = MediaFactory.create("ebook", id=2, title="E", author="A", year=2000, file_size=1.0, format="EPUB")
    audio = MediaFactory.create("audiobook", id=3, title="A", author="A", year=2000, duration=60, narrator="N")
...
#  Core Checkout Mechanics & Edge Cases 
...
def test_member_borrow_limit_raises(sample_member):
    ...
    item1 = MediaFactory.create("ebook", id=1, title="E1", author="A", year=2000, file_size=1, format="PDF")
    item2 = MediaFactory.create("ebook", id=2, title="E2", author="A", year=2000, file_size=1, format="PDF")
    item3 = MediaFactory.create("ebook", id=3, title="E3", author="A", year=2000, file_size=1, format="PDF")
...
# Polymorphic Late Fee Behavior 

@pytest.mark.parametrize("media_type, kwargs, days_late, expected_fee", [
...
def test_polymorphic_late_fee_calculation(media_type, kwargs, days_late, expected_fee):
    item = MediaFactory.create(media_type, id=1, title="T", author="A", year=2020, **kwargs)
    assert item.calculate_late_fee(days_late) == pytest.approx(expected_fee)


#  Observer Pattern (Due Date Alerts) 
...
# Serialization (Lifecycle Persistence) 
